{
  ...
  nodePtyVersion = sources.node-pty.version;

  betterSqlite3Src = sources.better-sqlite3.src;
  nodePtySrc = sources.node-pty.src;
in
  stdenv.mkDerivation {
    pname = "codex-desktop";
    version = sources.codex-dmg.version;

    src = sources.codex-dmg.src;
